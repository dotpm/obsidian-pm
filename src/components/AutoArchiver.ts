...
import { type Project, today, isTerminalStatus, tn } from '@dotpm/core'
...
export class AutoArchiver {
  ...
  async check(): Promise<void> {
    ...
    try {
      ...
      if (tasks) {
        this.plugin.showNotice(tn('archive.sweepDone', tasks, { projects: tn('count.projects', plans.length) }))
      }
    } finally {
      this.running = false
    }
