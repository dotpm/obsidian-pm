...
- Other apps on the same computer can read and edit tasks and create projects over a local HTTP and MCP server when it is turned on in settings
- Tasks can be listed, searched and edited, and projects created, from a terminal with the `dotpm` command, installed from npm as `@dotpm/cli`
- A tab with the release notes opens after the plugin updates
- Opening the release notes after an update can be turned off in settings
